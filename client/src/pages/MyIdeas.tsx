...
import { Lightbulb, Heart, MessageSquare, Plus, ArrowLeft } from "lucide-react";
...
export default function MyIdeas() {
  ...
  useEffect(() => {
    if (!user) return;
    getUserPosts(user.uid, "idea").then((data) => {
      setPosts(data);
      setLoading(false);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">All your posted ideas</p>
            </div>
          </div>
          <Link href="/feed">
        ...
        {loading ? (
          ...
          <Card className="glass-card border-white/5">
            <CardContent className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <p className="text-muted-foreground font-medium">No ideas posted yet</p>
              <Link href="/feed">
                <Button variant="outline" size="sm">Post your first idea</Button>
              </Link>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {post.domains?.map((domain) => (
                          <Badge key={domain} variant="outline" className="text-xs bg-primary/5 border-primary/20 text-primary">
                            {domain}
                          </Badge>
                        ))}
                      </div>
